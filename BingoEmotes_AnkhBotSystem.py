...
def Init():
    ...
    if Parent.IsLive() == True and MySettings.TriggerWord == 1:



        
        MySettings.Trigger = 1
        ...
        MySettings.word = random.choice(nouns)
        Parent.SendStreamWhisper(MySettings.Streamer,MySettings.word)
        Parent.SendTwitchMessage(MySettings.Start)
        MySettings.TriggerWord = 0
...
def Execute(data):
    ...
    if data.IsChatMessage() and data.GetParam(0).lower():
        ...
        if startCheck and  Parent.HasPermission(data.User, MySettings.Permission, MySettings.PermissionInfo):
            ...
            if data.GetParam(0) == MySettings.word:
                #give user points
                Parent.AddPoints(data.User,data.UserName,MySettings.Payout)
                #announce winner.
                Parent.SendTwitchMessage(MySettings.BaseResponse.format(data.UserName,MySettings.word,MySettings.Payout,Parent.GetCurrencyName()))
